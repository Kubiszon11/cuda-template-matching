...
__global__ void bounding_box(uint8_t* input, uint8_t* output, int width, int height, int widthA, int heightA, int widthAdec, int heightAdec, int widthAinc, int heightAinc, int widthA45, int heightA45, int* sample_check, int* top_left, int samples, int samplesdec, int samplesinc, int samples45);
...
__global__ void bounding_box(uint8_t* input, uint8_t* output, int width, int height, int widthA, int heightA, int widthAdec, int heightAdec, int widthAinc, int heightAinc, int widthA45, int heightA45, int* sample_check, int* top_left, int samples, int samplesdec, int samplesinc, int samples45) {
	...
	if (x < width && y < height) {
		...
		for (int i = samples; i < samples + samples45; i++) {
			int box_x = top_left[i * 2];
			int box_y = top_left[i * 2 + 1];
			//atomicMax(a, i);
			// Sprawdzanie, czy piksel znajduje się na krawędziach któregokolwiek bounding boxa
			if (x >= box_x && x <= box_x + heightA45 &&
				y >= box_y && y <= box_y + heightA45 &&
				(x == box_x || x == box_x + heightA45 || y == box_y || y == box_y + heightA45)) {
				// Rysuj krawędzie prostokąta
				output[idx] = 0;     // Czerwony
				output[idx + 1] = 255;   // Zielony
				output[idx + 2] = 0;   // Niebieski
		...
		for (int i = samples + samples45; i < samples + samples45 + samplesdec; i++) {
			int box_x = top_left[i * 2];
			int box_y = top_left[i * 2 + 1];
			//atomicMax(a, i);
			// Sprawdzanie, czy piksel znajduje się na krawędziach któregokolwiek bounding boxa
			if (x >= box_x && x <= box_x + heightAdec &&
				y >= box_y && y <= box_y + heightAdec &&
				(x == box_x || x == box_x + heightAdec || y == box_y || y == box_y + heightAdec)) {
				// Rysuj krawędzie prostokąta
				output[idx] = 0;     // Czerwony
				output[idx + 1] = 255;   // Zielony
				output[idx + 2] = 0;   // Niebieski
		...
		for (int i = samples + samples45 + samplesdec; i < samples + samples45 + samplesdec + samplesinc; i++) {
			int box_x = top_left[i * 2];
			int box_y = top_left[i * 2 + 1];
			//atomicMax(a, i);
			// Sprawdzanie, czy piksel znajduje się na krawędziach któregokolwiek bounding boxa
			if (x >= box_x && x <= box_x + heightAinc &&
				y >= box_y && y <= box_y + heightAinc &&
				(x == box_x || x == box_x + heightAinc || y == box_y || y == box_y + heightAinc)) {
				// Rysuj krawędzie prostokąta
				output[idx] = 0;     // Czerwony
				output[idx + 1] = 255;   // Zielony
				output[idx + 2] = 0;   // Niebieski
